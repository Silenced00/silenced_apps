...
var b=["squarespace-util","squarespace-search","squarespace-image-loader","squarespace-video-loader","squarespace-audio-player","squarespace-gallery-ng","squarespace-census","thirdparty-modernizr","squarespace-commerce"];
...
},initializeCommerce:function(d){if(!Static.SQUARESPACE_CONTEXT.hideShoppingCart){var a=function(g,l,q,s,n){g.removeAttribute("data-unselected-options");
g.removeAttribute("data-selected-variant");
g.removeAttribute("data-variant-in-stock");
var r=[];
var k=null;
var e=false;
var j=g.one(".variant-out-of-stock");
if(j){j.remove()
}var f={};
q.each(function(i){var u=i.get("value");
var t=i.getAttribute("data-variant-option-name");
if(u!="none"){f[t]=u
}else{r.push(t)
}},this);
if(r.length===0){for(var m=0;
m<l.length;
m++){var h=l[m];
var p=true;
for(var o in f){if(f[o]!=h[o]){p=false;
break
}}if(p){k=h;
if(h.unlimited||h.qtyInStock>0){e=true
}break
}}if(!k&&s){s.set("text","Unavailable")
}else{if(s){if(k.onSale){s.setHTML(d.Squarespace.Commerce.moneyString(k.salePrice));
s.append(d.Node.create('<span> </span><span class="original-price">'+d.Squarespace.Commerce.moneyString(k.price)+"</span>"))
}else{s.setHTML(d.Squarespace.Commerce.moneyString(k.price))
}}}if(k&&!e){g.append(d.Node.create('<div class="variant-out-of-stock">Out of stock.</div>'))
}}else{if(s&&(s.getHTML()!==n)){s.setHTML(n)
}}g.setAttribute("data-unselected-options",JSON.stringify(r));
if(k){g.setAttribute("data-selected-variant",JSON.stringify(k))
}if(e){g.setAttribute("data-variant-in-stock",e)
}};
var c=d.all(".product-variants[data-variants]");
c.each(function(e){var h=JSON.parse(e.getAttribute("data-variants"));
var g=e.all("select");
var i=e.siblings(".product-price").item(0);
var f;
if(i){f=i.getHTML()
}a(e,h,g,i,f);
g.after("change",function(j){a(e,h,g,i,f)
},this)
},this);
d.all(".sqs-add-to-cart-button").each(function(e){if(Static.SQUARESPACE_CONTEXT.websiteSettings.storeSettings.expressCheckout){e.setContent("Purchase");
e.setData("originalLabel","Purchase")
}else{e.setData("originalLabel","Add to Cart")
}});
d.all(".sqs-add-to-cart-button-wrapper").setStyle("visibility","visible");
var b=d.one(".sqs-cart-dropzone");
if(d.Lang.isNull(b)){b=d.DB.DIV("absolute-cart-box");
b.appendTo(d.one("body"))
}if(d.Lang.isNull(d.Cookie.get("CART"))){d.Squarespace.ShoppingCart=new d.Squarespace.Widgets.PillShoppingCart({expressCheckout:Static.SQUARESPACE_CONTEXT.websiteSettings.storeSettings.expressCheckout,useLightCart:Static.SQUARESPACE_CONTEXT.websiteSettings.storeSettings.useLightCart}).render(b)
}else{d.Data.get({url:"/api/open/GetShoppingCart",success:function(e){d.Squarespace.ShoppingCart=new d.Squarespace.Widgets.PillShoppingCart({shoppingCart:e.shoppingCart,expressCheckout:Static.SQUARESPACE_CONTEXT.websiteSettings.storeSettings.expressCheckout,useLightCart:Static.SQUARESPACE_CONTEXT.websiteSettings.storeSettings.useLightCart}).render(b)
}})
}d.all(".sqs-add-to-cart-button").each(function(e){e.on("click",function(r){r.halt();
var s=e.getAttribute("data-item-id");
var m=null;
var j=d.one(e.get("parentNode").siblings(".product-variants").item(0));
...
}}}}d.Squarespace.ShoppingCart.showError(l);
return
}else{if(!n){d.Squarespace.ShoppingCart.showError("Sorry, that item variant is unavailable. Please select another variant.");
return
}else{if(!f){d.Squarespace.ShoppingCart.showError("Sorry, we don't have enough of that item available.");
return
}else{m=n
}}}}var h=d.one('input[data-item-id="'+s+'"][data-name="payWhatYouWantPrice"]');
var u;
if(h){payWhatYouWantPriceString=h.get("value").replace(/\s+/g,"").replace(/\,/g,"");
var w=payWhatYouWantPriceString.split(".");
if(w.length>2||(w[1]&&w[1].length>2)){d.Squarespace.ShoppingCart.showError("Payment price inavlid.");
return
}u=parseFloat(payWhatYouWantPriceString)
}var q=e.get("parentNode").siblings(".product-quantity-select").item(0);
var v;
if(q){v=1;
v=d.one(q).one("select").get("value")
}e.setContent("Adding...");
d.Squarespace.ShoppingCart.addItem(s,m,v,u,function(){e.setContent("Added!");
e.get("parentNode").addClass("cart-added");
e.detach("click")
},function(){e.setContent(e.getData("originalLabel"))
})
})
})
...
a.namespace("Squarespace").CensusDataType={TOTAL:1,PAGE:2,BROWSER:3,OS:4,COUNTRY:5,REFERRER:6,SEARCH:7};
...
a.namespace("Squarespace").ImportingSources={V5:1,WORDPRESS:2,TUMBLR:3,POSTEROUS:4,BLOGGER:5,FLICKR:6,ETSY:7,SHOPIFY:8,BIG_CARTEL:9};
